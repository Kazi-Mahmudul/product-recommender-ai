"""fix battery capacity extraction

Revision ID: fc5337322210
Revises: 8ca927e98307
...
revision = 'fc5337322210'
down_revision = '8ca927e98307'
branch_labels = None
depends_on = None
...
def extract_battery_mah(battery_str):
    ...
    if 'WH' in battery_str.upper():
        # Approximate conversion: 1 Wh ≈ 270 mAh
        battery_value *= 270
...
def upgrade() -> None:
    ...
    for phone in phones:
        try:
            phone_id = phone[0]
            price = float(phone[1]) if phone[1] else 0
            ram_str = phone[2]
            battery_str = phone[3]
            
            # Extract values
            ram_gb = extract_ram_gb(ram_str)
            battery_mah = extract_battery_mah(battery_str)
            
            # Calculate performance score
            performance_score = calculate_performance_score(ram_gb, battery_mah, price)
            
            # Update the phone record
            connection.execute(
                text("UPDATE phones SET performance_score = :score WHERE id = :id"),
                {"score": performance_score, "id": phone_id}
            )
            
            logger.info(f"Updated phone {phone_id}: RAM={ram_gb}GB, Battery={battery_mah}mAh, Score={performance_score}")
            
        except Exception as e:
            logger.error(f"Error processing phone {phone[0]}: {str(e)}")
            # Set score to 0 for failed calculations
            connection.execute(
                text("UPDATE phones SET performance_score = 0 WHERE id = :id"),
                {"id": phone[0]}
            )


def downgrade() -> None:
    # Reset all performance scores to NULL
    connection = op.get_bind()
