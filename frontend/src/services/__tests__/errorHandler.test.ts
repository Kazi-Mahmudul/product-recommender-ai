...
const mockChatContextManager = ChatContextManager as jest.Mocked<typeof ChatContextManager>;
...
describe('ErrorHandler', () => {
  ...
  describe('handleError', () => {
    ...
    it('should handle string errors', () => {
      const result = ErrorHandler.handleError('Something went wrong', mockContext);
  ...
  describe('handleSuggestionError', () => {
    it('should return fallback suggestions', () => {
      ...
      const phones = [{ name: 'Test Phone', price_original: 50000 }];
      
      const suggestions = ErrorHandler.handleSuggestionError(error, phones, mockContext);
  ...
  describe('handleDrillDownError', () => {
    it('should provide alternative commands', () => {
      ...
      const result = ErrorHandler.handleDrillDownError(error, 'invalid_command', mockContext);
    ...
    it('should suggest valid drill-down commands', () => {
      ...
      expect(result.alternatives.some(alt => alt.includes('specs'))).toBe(true);
      expect(result.alternatives.some(alt => alt.includes('chart'))).toBe(true);
    });
  });
  ...
  describe('handleComparisonError', () => {
    it('should provide fallback comparison content', () => {
      ...
      const phones = [{ name: 'Phone 1', brand: 'Brand 1', price: '50000' }];
      const features = [{ label: 'Price', raw: [50000] }];

      const result = ErrorHandler.handleComparisonError(error, phones, features);
  ...
  describe('handleContextError', () => {
    it('should return fresh context on error', () => {
      ...
      mockChatContextManager.initializeContext.mockReturnValue(mockFreshContext);
  ...
  describe('error classification', () => {
    it('should classify network errors correctly', () => {
      ...
      networkErrors.forEach(errorMessage => {
        const result = ErrorHandler.handleError(new Error(errorMessage), mockContext);
        expect(result.fallbackMessage).toBeDefined();
      });
    });
