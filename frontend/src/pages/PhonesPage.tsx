...
const PhonesPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-neutral-50 to-white dark:from-neutral-900 dark:to-neutral-800 pt-16 md:pt-24 pb-8 md:pb-16 px-3 md:px-4 sm:px-8">
      {/* Header Section */}
      ...
      {/* Filters Section */}
      <div className="max-w-7xl mx-auto mb-4 md:mb-8">
        <div className="bg-white dark:bg-card rounded-2xl md:rounded-3xl shadow-soft p-3 md:p-6">
            ...
            <div className="relative flex-1 max-w-md" ref={searchRef}>
              <form onSubmit={handleSearchSubmit}>
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-8 md:pl-10 pr-8 md:pr-10 py-2 md:py-3 rounded-lg md:rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand/30 text-sm md:text-base"
                    autoComplete="off"
                  />
                  {searchQuery ? (
                    <button
                      ...
                      className="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200"
                    >
                      <X size={16} className="md:size-18" />
                    </button>
                  ) : (
                    <div className="absolute left-2 md:left-3 top-1/2 -translate-y-1/2 text-neutral-400">
              ...
              {searchFocused && searchQuery.trim().length >= 2 && (
                <div className="absolute left-0 right-0 top-full mt-1 md:mt-2 z-50 bg-white dark:bg-neutral-900 rounded-xl md:rounded-2xl shadow-soft-lg border border-neutral-200 dark:border-neutral-700/30 p-2 md:p-3">
                  <div className="max-h-60 md:max-h-80 overflow-y-auto rounded-lg md:rounded-xl">
                    {searchResults.length > 0 ? (
                      <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
                        {searchResults.map((result) => (
                          <div
                            key={result.id}
                            className="flex items-center gap-2 md:gap-3 p-2 hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-md md:rounded-lg cursor-pointer transition-colors duration-150"
                            onClick={() => handleResultClick(result.slug || `phone-${result.id}`)}
                          >
                            <div className="w-8 h-8 md:w-10 md:h-10 bg-neutral-100 dark:bg-neutral-800 rounded-md flex-shrink-0 flex items-center justify-center overflow-hidden">
                              {result.img_url ? (
                                <img
                                  src={result.img_url}
                                  alt={result.name}
                                ...
                                <Smartphone
                                  size={14}
                                  className="md:size-16 text-neutral-400"
                                />
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs md:text-sm font-medium text-neutral-800 dark:text-neutral-200 truncate">
                                {result.name}
                              </p>
                      ...
                      !isSearching && (
                        <div className="py-2 md:py-3 px-2 text-center text-xs md:text-sm text-neutral-500 dark:text-neutral-400">
            ...
            >
              {activeFiltersCount > 0 ? (
                <>
                  ...
                  <span className="hidden xs:inline">Filters ({activeFiltersCount})</span>
                  <span className="xs:hidden">({activeFiltersCount})</span>
                </>
              ) : (
                <>
                  <SlidersHorizontal size={16} className="md:size-18" />
      ...
      <div className="max-w-7xl mx-auto">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 md:py-24">
            <div className="w-12 h-12 md:w-16 md:h-16 border-4 border-primary/20 border-t-primary rounded-full animate-spin mb-3 md:mb-4"></div>
          ...
          <>
            {/* Results Count */}
            <div className="mb-4 md:mb-6 text-xs md:text-sm text-neutral-500 dark:text-neutral-400">
              {phones.length > 0 ? (
                <>
                  Showing{" "}
                ...
                <div className="text-center py-6 md:py-8">
                  <div className="text-base md:text-lg font-medium text-neutral-800 dark:text-white mb-2">
                    No phones match your filters
                  </div>
                  <p className="text-neutral-500 dark:text-neutral-400 mb-3 md:mb-4">
                    Try adjusting your filters or clearing them to see more
                    results.
                  </p>
                  <button
                    onClick={handleClearFilters}
                    className="px-3 py-2 md:px-4 md:py-2 bg-brand text-white rounded-lg hover:bg-brand/90 transition-colors text-sm md:text-base"
                  >
                    Clear All Filters
                  </button>
                </div>
