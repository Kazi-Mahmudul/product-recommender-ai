...
const EnhancedComparison: React.FC<EnhancedComparisonProps> = ({
  ...
  const chartData = useMemo(() => {
    return features.map(feature => {
      ...
      phones.forEach((phone, index) => {
        dataPoint[phone.name] = feature.percent[index];
        dataPoint[`${phone.name}_raw`] = feature.raw[index];
        dataPoint[`${phone.name}_color`] = phoneColors[index % phoneColors.length];
      });
  ...
  const insights = useMemo(() => {
    ...
    features.forEach(feature => {
      ...
      if (maxPhone && maxValue !== null && maxValue !== undefined) {
        insights.push(`${maxPhone.name} leads in ${feature.label} with ${maxValue}${getUnit(feature.key)}`);
      }
    });
              ...
              return (
                <div key={index} className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    <div className="font-bold">{entry.value.toFixed(1)}%</div>
                    <div className="text-sm opacity-75">
                      {rawValue}{getUnit(feature?.key || '')}
                    </div>
                  </div>
                </div>
              );
  ...
  const CustomBarLabel = ({ x, y, width, height, value, fill }: any) => {
    if (value > 5) { // Only show label if bar is large enough
      return (
        <text
          x={x + width / 2}
          y={y + height / 2}
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Interactive comparison of {phones.length} phones across {features.length} features
            </p>
          </div>
        </div>
        ...
        {insights.length > 0 && (
          <div className={`p-4 rounded-lg ${
            ...
            <div className="flex items-center gap-2 mb-3">
              <Award className={darkMode ? 'text-yellow-400' : 'text-yellow-500'} size={20} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {features.slice(0, 3).map((feature, featureIndex) => (
                <div key={featureIndex} className="flex justify-between items-center">
                  ...
                  <span className="font-medium">
                    {feature.raw[index]}{getUnit(feature.key)}
                  </span>
                </div>
              ))}
            </div>
        ...
        {onAddPhone && phones.length < 5 && (
          <div
            ...
            onClick={() => {/* This would trigger a phone selection modal */}}
          >
            <Plus size={32} className={darkMode ? 'text-gray-400' : 'text-gray-500'} />
            <p className={`mt-2 text-sm font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              Add Phone
            </p>
          </div>
        )}
            ...
            >
              ...
              {phones.map((phone, index) => (
                <Bar
                  ...
                  radius={index === phones.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]}
        ...
        {selectedFeature && (
          <div className={`mt-6 p-4 rounded-lg ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {phones.map((phone, index) => {
                const feature = features.find(f => f.label === selectedFeature);
