 ...
 */
export function useComparisonState(initialPhoneIds: number[] = []): [ComparisonState, ComparisonActions] {
  ...
  const fetchPhonesInternalRef = useRef<(phoneIds: number[], isRetry?: boolean) => Promise<void>>();

  // Validate current comparison
  const isValidComparison = validateComparisonPhoneIds(selectedPhoneIds).isValid;
   ...
   */
  const fetchPhonesInternal = useCallback(async (phoneIds: number[], isRetry = false): Promise<void> => {
    ...
    try {
      ...
      if (failedIds.length > 0) {
        console.warn(`Failed to fetch phones with IDs: ${failedIds.join(', ')}`);
      ...
      if (error.statusCode === 422) {
        errorMessage = 'Some phone IDs are invalid or no longer available. Please try different phones.';
      } else if (error.statusCode === 404) {
        errorMessage = 'The requested phones could not be found. They may have been removed.';
      } else if (error.statusCode === 400) {
        errorMessage = 'Invalid request. Please check the phone selection and try again.';
      } else if (error.message?.includes('ERR_INSUFFICIENT_RESOURCES')) {
        errorMessage = 'Server is experiencing high load. Please try again in a moment.';
   ...
   */
  const addPhone = useCallback((phoneId: number) => {
   ...
   */
  const replacePhone = useCallback((oldPhoneId: number, newPhoneId: number) => {
    ...
    const newPhoneIds = selectedPhoneIds.map(id => id === oldPhoneId ? newPhoneId : id);
    const validation = validateComparisonPhoneIds(newPhoneIds);
